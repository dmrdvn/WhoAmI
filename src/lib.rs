...
const QUESTIONS: [Question; 5] = [
    Question {
        text: "Soru 1: ...",
        options: &["Cevap A", "Cevap B"],
        correct_option: 1,
        points: 10,
    },
    Question {
        text: "Soru 2: ...",
        options: &["Cevap X", "Cevap Y"],
        correct_option: 2,
        points: 15,
    },
    Question {
...
fn mint(
    ...
    let (txid, tkid) = STATE.with(|state| {
        ...
        // Soruları kontrol et ve puanları hesapla
        let mut total_points = 0;
        for (i, &answer) in answers.iter().enumerate() {
            if answer == QUESTIONS[i].correct_option {
                total_points += QUESTIONS[i].points;
            }
        }

        // NFT'yi oluştur ve özel özellikleri belirle
        let new_id = state.nfts.len() as u64;
        let special_feature = calculate_special_feature(total_points);
        let nft = Nft {
            owner: to,
            approved: None,
            id: new_id,
...
// Puanlara göre özel özellikleri hesapla
fn calculate_special_feature(total_points: u64) -> Option<String> {
    // Özel özellikleri hesaplayın, örneğin:
    if total_points >= 40 {
        Some("HighQuality".to_string())
    } else if total_points >= 20 {
        Some("MediumQuality".to_string())
    } else {
        None
    }
}
